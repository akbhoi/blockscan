...
6.  **Verify Coverage:** Run coverage reports using `cargo tarpaulin` or `llvm-cov` if configured.
...
8.  **Commit Code Changes:**
    ...
    -   Propose a clear, concise commit message e.g, `feat(cli): Add --plain output mode`.
    -   Perform the commit.
...
11. **Commit Plan Update:**
    ...
    -   **Action:** Commit this change with a descriptive message (e.g.,
        `conductor(plan): Mark task 'Implement HTTP crawler semaphore' as complete`).
...
2.  **Code Review Corrections (`conductor-review`):** If issues are identified
    during or after a code review, trigger a review. The review agent will automatically append a `Review Fixes` phase to `plan.md`.
3.  **Logical State Reversions (`conductor-revert`):** If a task implementation
    is fundamentally flawed, use revert. This safely rolls back associated git
    commits and resets the task state in `plan.md` back to pending `[ ]`.
...
-   [ ] Strict Rust type safety and memory safety enforced
-   [ ] No clippy warnings (`cargo clippy -- -D warnings`)
-   [ ] Formatted cleanly (`cargo fmt --check`)
